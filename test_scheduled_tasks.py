...
from scheduled_tasks import ScheduledTasks, TZ, next_due, spec_from, schedule_intent
...
class ScheduledBotTests(unittest.TestCase):
    ...
    def test_due_reminder_uses_delivery_queue(self):
        self.create()
        b = self.make_bot()
        with patch('scheduled_tasks.time.time', return_value=self.now + 300):
    ...
    def test_ai_execution_is_isolated_and_cannot_schedule(self):
        ...
        b = self.make_bot()
        with patch('scheduled_tasks.time.time', return_value=self.now + 300), patch('bot.chat_complete', return_value=('晴天', {})) as complete:
            b.run_scheduled(['group1'])
    ...
    def process_as(self, sender, prompt, mode='send'):
        ...
        def complete(*args, **kwargs):
            ...
            return '无任务工具', {}
        with patch('bot.time.time', return_value=self.now), patch('bot.chat_complete', side_effect=complete):
