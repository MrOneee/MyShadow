...
from selective_reply import SelectiveReply
...
class DecisionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.clock=patch('participation_decision.time.time',return_value=self.now).start()
    ...
    def test_proactive_generation_never_resolves_fake_message_sender(self):
        ...
        with patch('bot.chat_complete',return_value=('选路线可以先看当天的时间余量。',{})) as complete:
            b.process_group('111@chatroom')
    ...
    def test_proactive_freshness_uses_original_anchor_not_queue_time(self):
        ...
        def snapshot(_):
            ...
            c.commit();return c
        with patch('bot.databases',return_value=(Path('/fake'),[Path('/fake/message/message_0.db')])),patch('bot.snapshot',side_effect=snapshot):
