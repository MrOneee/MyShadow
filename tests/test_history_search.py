...
from myshadow.history_search import HistorySearch, boundary
...
class HistoryTests(unittest.TestCase):
    def setUp(self):
        ...
        def snap(shard):
            ...
            return c
        for patcher in [patch('myshadow.history_search.snapshot',side_effect=snap),
                        patch('myshadow.history_search.databases',return_value=(Path('/db'),[Path('/db/message/message_0.db')]))]:
    ...
    def test_sender_is_resolved_only_in_current_group(self):
        ...
        with patch('myshadow.history_search.member_names',return_value={'alice':['小李'],'bob':['小王']}) as names:
    ...
    def test_ambiguous_sender_is_not_guessed(self):
        with patch('myshadow.history_search.member_names',return_value={'alice':['同名'],'bob':['同名']}):
    ...
    def test_scan_budget_reports_partial(self):
        ...
        with patch('myshadow.history_search.SCAN_LIMIT',1):result=self.query()
    ...
    def test_snapshot_closed_and_failure_not_reported_as_no_history(self):
        ...
        with patch('myshadow.history_search.snapshot',side_effect=RuntimeError('unreadable')):
            result=self.query()
    ...
    def test_cross_shard_duplicates_removed(self):
        self.add('同一条消息')
        with patch('myshadow.history_search.databases',return_value=(Path('/db'),[Path('/db/message/message_0.db'),Path('/db/message/message_1.db')])):
            result=self.query()
        self.assertEqual(len(result['messages']),1)
    ...
    def test_bot_offers_history_tool_without_other_tools(self):
        ...
        def complete(*args,**kwargs):
            ...
            return '这次没查到。',{}
        with patch('myshadow.bot.chat_complete',side_effect=complete),patch('myshadow.bot.time.time',return_value=self.now):
