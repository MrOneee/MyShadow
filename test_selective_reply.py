...
from bot import Bot, decode, message_xml
from selective_reply import SelectiveReply, addressed, conversation_text
...
class ParticipationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.clock = patch('selective_reply.time.time', return_value=1000).start()
    ...
    def test_fresh_database_cancels_ambient_reply_only(self):
        ...
        def database(sender):
            ...
            c.commit()
            return c
        with patch('bot.databases',return_value=(Path('/fake'),[Path('/fake/message/message_0.db')])), patch('bot.snapshot',side_effect=lambda _:database('bot')):
            self.assertTrue(b.participation_current(self.replies()[0]['id']))
        with patch('bot.databases',return_value=(Path('/fake'),[Path('/fake/message/message_0.db')])), patch('bot.snapshot',side_effect=lambda _:database('bob')):
        ...
        with patch('bot.snapshot') as snap:
            self.assertTrue(b.participation_current(self.replies()[0]['id']))
