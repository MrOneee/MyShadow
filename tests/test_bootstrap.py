...
import myshadow.bootstrap as bootstrap
...
class BootstrapTests(unittest.TestCase):
    ...
    def test_environment_overrides_mode_and_name(self):
        with patch.dict(bootstrap.os.environ, {'WECHAT_NAME': 'New name', 'BOT_MODE': 'preview'}), patch('myshadow.ai_client.AIClient'), patch.object(bootstrap, 'snapshot'), patch.object(bootstrap, 'extract_keys') as extract:
    ...
    def test_stale_keys_are_refreshed(self):
        with patch('myshadow.ai_client.AIClient'), patch.object(bootstrap, 'snapshot', side_effect=ValueError('stale')), patch.object(bootstrap, 'extract_keys') as extract:
            self.assertEqual(self.check(), 'ready')
