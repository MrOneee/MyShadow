...
from myshadow.bot import Bot, configured_groups, estimate_tokens, prompt_for, table_for
from myshadow.wechat_db import checksum, committed_frames
...
class RoutingTests(unittest.TestCase):
    ...
    def test_cross_group_delivery_activates_safety_lock(self):
        ...
        with patch('myshadow.bot.time.sleep'), patch('myshadow.bot.private_json') as write_lock:
...
class ContextTests(unittest.TestCase):
    def setUp(self):
        ...
        self.patches = [patch('myshadow.bot.databases', return_value=(Path('/db'), [Path('/db/message/message_0.db'), Path('/db/message/message_1.db')])),
                        patch('myshadow.bot.snapshot', side_effect=lambda rel: self.connections[rel])]
    ...
    def test_related_member_names_require_unambiguous_full_roster(self):
        ...
        with patch('myshadow.bot.member_names', return_value={'wxid_b': ['Bob'], 'x': ['Same'], 'y': ['Same']}):
            self.assertEqual(self.bot.related_senders(self.trigger, []), [{'sender': 'wxid_b', 'name': 'Bob'}])
        with patch('myshadow.bot.member_names', return_value={}):
