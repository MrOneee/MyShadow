...
from bot import Bot
from native_stickers import NativeStickers, TransientStickerUIError
...
class NativeSelectionTests(unittest.TestCase):
    ...
    def test_transient_search_failure_reprepares_once(self):
        ...
        with patch('native_stickers.time.sleep'):
    ...
    def test_transient_retry_is_bounded_and_returns_no_selection(self):
        ...
        with patch('native_stickers.time.sleep'), self.assertRaises(TransientStickerUIError):
    ...
    def test_ui_classifies_x11_error_and_handles_empty_stderr(self):
        ...
        with patch('native_stickers.subprocess.run',return_value=Mock(returncode=1,stderr='Traceback\nBadDrawable: vanished')):
            with self.assertRaises(TransientStickerUIError):lib.ui('prepare',{})
        with patch('native_stickers.subprocess.run',return_value=Mock(returncode=1,stderr='')):
    ...
    def test_random_choice_uses_only_first_five_cells(self):
        ...
        with patch('native_stickers.secrets.choice',side_effect=lambda cells:cells[-1]) as choose:
